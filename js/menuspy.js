/*! 20190811 Modified by Fangzhou from MenuSpy v1.3.0 (Jan 31 2018) - http://leocs.me/menuspy/ - Copyright (c) 2018 Leonardo Santos; MIT License */
(function (global, factory) {
  ...
  MenuSpy.prototype.cacheItems = function cacheItems() {
    this.scrollItems = this.menuItems.map(function (elm) {
      var target = elm.dataset.target ? document.querySelector(elm.dataset.target) : document
        .getElementById(elm.hash.slice(1));
      if (target) {
  ...
  MenuSpy.prototype.activateItem = function activateItem(inViewElm) {
    ...
    if (!inViewElm) {
      this.scrollItems.forEach(function (item) {
        return utils.removeClass(item.elm, activeClass);
      });
      this.lastInViewElm = null;
    ...
    if (this.lastInViewElm !== inViewElm.target) {
      ...
      this.scrollItems.forEach(function (item) {
        utils.removeClass(item.elm, activeClass);

        if (item.target === inViewElm.target) {
          utils.addClass(item.elm, activeClass);
